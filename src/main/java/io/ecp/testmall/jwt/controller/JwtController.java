package io.ecp.testmall.jwt.controller;
...
public class JwtController {
    ...
    public Map<String, Object> refresh(@RequestHeader("Authorization") String authHeader, String refreshToken) {
        ...
        } else if (!authHeader.startsWith(JwtConst.JWT_TYPE)) {
            throw new CustomJwtException("BEARER 로 시작하지 않는 올바르지 않은 토큰 형식입니다");
        }

        String accessToken = JwtUtils.extractToken(authHeader);
        ...
        String newAccessToken = JwtUtils.generateToken(claims, JwtConst.ACCESS_EXP_TIME);
        ...
        if (expTime <= 60) {
            newRefreshToken = JwtUtils.generateToken(claims, JwtConst.REFRESH_EXP_TIME);
        }
